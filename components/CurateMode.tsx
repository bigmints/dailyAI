...
const CurateMode: React.FC<CurateModeProps> = ({ onArticleAdded }) => {
  ...
  return (
    <div className="max-w-xl mx-auto p-8 bg-white border border-zinc-100 rounded-3xl shadow-sm mb-12 animate-slide-up">
      <div className="flex items-center gap-3 mb-6">
        <div className="p-2 bg-indigo-50 text-indigo-600 rounded-xl">
          <Sparkles size={20} />
        </div>
        <h2 className="text-xl font-bold text-zinc-900">Curate New Content</h2>
      </div>
      ...
      <div className="space-y-4">
        <div>
          <input 
            ...
            className="w-full px-4 py-3 bg-zinc-50 border border-zinc-200 rounded-2xl focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 transition-all text-zinc-900"
          />
        </div>
        ...
        <button 
          ...
          className="w-full py-4 bg-zinc-900 text-white rounded-2xl font-bold flex items-center justify-center gap-2 hover:bg-zinc-800 disabled:opacity-50 disabled:cursor-not-allowed transition-all shadow-lg shadow-zinc-100"
        >
          {isGenerating ? (
